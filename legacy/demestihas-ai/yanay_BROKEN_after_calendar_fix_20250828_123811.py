...
{{
    ...
    def contains_calendar_intent(self, message):
        ...
        calendar_keywords = [
            'calendar', 'schedule', 'free', 'available', 'busy',
            'today', 'tomorrow', 'monday', 'tuesday', 'wednesday', 
            'thursday', 'friday', 'saturday', 'sunday',
            'morning', 'afternoon', 'evening', 'night',
            'week', 'month', 'time', 'when', 'what time',
            # Event/meeting references - CRITICAL FIX
    ...
    async def process_message(self, user_message: str, user_id: str, user_name: str = "User") -> str:
        ...
        # CRITICAL: Check for calendar intent FIRST before other classification
        if self.contains_calendar_intent(user_message):
            logger.info(f"Calendar intent detected: {user_message}")
            # Return calendar-specific response for now
            response = "📅 Calendar queries are being processed. Please try: 'Show my tasks' for now."
