...
import redis
from .notion_schema_adapter import create_notion_adapter
...
class LycoEisenhowerAgent:
    ...
    def __init__(self):
        ...
        if self.notion and self.notion_database_id:
            try:
                self.schema_adapter = create_notion_adapter(self.notion, self.notion_database_id)
                logger.info("✅ Schema adapter initialized successfully")
                logger.info(self.schema_adapter.get_mapping_report())
            except Exception as e:
                logger.error(f"Failed to initialize schema adapter: {e}")
                self.schema_adapter = None
...
If unclear, use urgency=3, importance=3."""
    ...
    def save_task_to_notion(self, task: Task) -> bool:
        """Save task to Notion database using schema adapter"""
        if not self.notion or not self.notion_database_id:
            logger.warning("Notion not configured")
            return False
        
        try:
            # Convert Task object to dictionary format expected by adapter
            task_data = {
                'title': task.title,
            ...
            # Use adapter to transform properties
            if self.schema_adapter:
                adapted_properties = self.schema_adapter.adapt_task_for_notion(task_data)
                logger.info(f"Adapted properties: {list(adapted_properties.keys())}")
            else:
                logger.warning("No schema adapter available, using fallback")
                adapted_properties = self._fallback_properties(task)
            
            # Create page with adapted properties
            response = self.notion.pages.create(
                parent={"database_id": self.notion_database_id},
                properties=adapted_properties
            )
            
            task.notion_id = response["id"]
            logger.info(f"✅ Task saved to Notion: {task.title}")
            ...
            if self.schema_adapter:
                logger.error(f"Schema mapping report:\n{self.schema_adapter.get_mapping_report()}")
            
            # Try fallback one more time
            try:
                logger.info("Attempting fallback save with minimal properties...")
                minimal_props = {"Name": {"title": [{"text": {"content": task.title}}]}}
                response = self.notion.pages.create(
                    parent={"database_id": self.notion_database_id},
                    properties=minimal_props
                )
                task.notion_id = response["id"]
                logger.info(f"✅ Fallback save successful: {task.title}")
                return True
            except Exception as fallback_error:
                logger.error(f"Even fallback save failed: {fallback_error}")
                return False
    
    def _fallback_properties(self, task: Task) -> dict:
        """Create fallback properties when adapter is not available"""
        props = {
            "Name": {"title": [{"text": {"content": task.title}}]},
            "Complete": {"checkbox": task.status == TaskStatus.COMPLETE}
        }
        
        if task.description:
            props["Notes"] = {"rich_text": [{"text": {"content": task.description}}]}
        
        if task.due_date:
            props["Due Date"] = {"date": {"start": task.due_date}}
        
        if task.context_tags:
            props["Context/Tags"] = {"multi_select": [{"name": tag} for tag in task.context_tags]}
