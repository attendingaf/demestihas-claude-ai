...
async function startServer(): Promise<void> {
    try {
        ...
        // Allowed databases for multi-tenancy
        ...
        // Helper to get graph name from request with validation
        const getGraphName = (database?: string): string => {
            if (!database) return defaultGraphName;
            if (!allowedDatabases.has(database)) {
                throw new Error(
                    `Invalid database: ${database}. Allowed: ${Array.from(allowedDatabases).join(", ")}`,
                );
            }
            return database;
        };
        ...
        app.post("/graph/query", authMiddleware, async (req, res) => {
            try {
                const { cypher, params, database } = req.body;
                if (!cypher)
                    return res
                        .status(400)
                        .json({ error: "Missing cypher query" });

                const graphName = getGraphName(database);
                const client = getDb();
                ...
                const relationshipsCreated =
                    (result as any).relationshipsCreated || 0;
        ...
        app.post("/graph/nodes", authMiddleware, async (req, res) => {
            try {
                const { label, properties, database } = req.body;
                if (!label)
                ...
                // Construct CREATE query with property setting
                const props = properties || {};
                const propEntries = Object.entries(props);

                let query = `CREATE (n:${label})`;
                if (propEntries.length > 0) {
                    const setClause = propEntries
                        .map(([key, _], idx) => `n.${key} = $prop${idx}`)
                        .join(", ");
                    query += ` SET ${setClause}`;
                }
                query += ` RETURN n`;
                ...
                const params: any = {};
                propEntries.forEach(([_, value], idx) => {
                    params[`prop${idx}`] = value;
                });
        ...
        app.post("/graph/relationships", authMiddleware, async (req, res) => {
            try {
                ...
                const relPropEntries = Object.entries(relProps);

                let query = `
            MATCH (a:${from_label}), (b:${to_label})
            WHERE ${Object.keys(from_match)
                        .map((k) => `a.${k} = $from_${k}`)
                        .join(" AND ")}
            AND ${Object.keys(to_match)
                        .map((k) => `b.${k} = $to_${k}`)
                        .join(" AND ")}
            CREATE (a)-[r:${relationship_type}]->(b)`;

                if (relPropEntries.length > 0) {
                    const setClause = relPropEntries
                        .map(([key, _], idx) => `r.${key} = $rprop${idx}`)
                        .join(", ");
                    query += ` SET ${setClause}`;
                }
                ...
                const params: any = {};
                Object.entries(from_match).forEach(
                    ([k, v]) => (params[`from_${k}`] = v),
                );
                Object.entries(to_match).forEach(
                    ([k, v]) => (params[`to_${k}`] = v),
                );
                relPropEntries.forEach(([_, value], idx) => {
                    params[`rprop${idx}`] = value;
                });
        ...
        app.get("/graph/search", authMiddleware, async (req, res) => {
            try {
                const { label, property, value, database } = req.query;
                ...
                const graphName = getGraphName(database as string | undefined);
                const client = getDb();
                const graph = client.selectGraph(graphName);
        ...
        app.get(
            "/graph/node/:label/:property/:value/relationships",
            authMiddleware,
            async (req, res) => {
                try {
        ...
        `;
                    ...
                    const [outResult, inResult, nodeResult] = await Promise.all(
                        [
                            graph.query(outQuery, {
                                params: { value },
                            } as any) as Promise<any>,
                            graph.query(inQuery, {
                                params: { value },
                            } as any) as Promise<any>,
                            graph.query(nodeQuery, {
                                params: { value },
                            } as any) as Promise<any>,
                        ],
                    );
        ...
        app.post("/graph/bulk", authMiddleware, async (req, res) => {
            try {
                const { nodes, relationships, database } = req.body;
                const graphName = getGraphName(database);
                const client = getDb();
                const graph = client.selectGraph(graphName);
                ...
                if (relationships && Array.isArray(relationships)) {
                    for (const rel of relationships) {
                        ...
                        WHERE ${Object.keys(rel.from.match)
                                    .map((k) => `a.${k} = $from_${k}`)
                                    .join(" AND ")}
                        AND ${Object.keys(rel.to.match)
                                    .map((k) => `b.${k} = $to_${k}`)
                                    .join(" AND ")}
                        CREATE (a)-[r:${rel.type}]->(b)`;
                            ...
                            const params: any = {};
                            Object.entries(rel.from.match).forEach(
                                ([k, v]) => (params[`from_${k}`] = v),
                            );
                            Object.entries(rel.to.match).forEach(
                                ([k, v]) => (params[`to_${k}`] = v),
                            );
                            relPropEntries.forEach(([_, value], idx) => {
                                params[`rprop${idx}`] = value;
