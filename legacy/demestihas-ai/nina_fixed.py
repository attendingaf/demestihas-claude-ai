...
class NinaSchedulingAgent:
    ...
    def _init_calendar(self):
        ...
        try:
            ...
            if os.path.exists(creds_path):
                ...
                self.viola_calendar_id = 'primary'  # Use primary calendar
                logger.info("✅ Nina: Google Calendar API initialized")
            else:
                logger.warning(f"Calendar credentials not found at {creds_path}")
        except Exception as e:
            logger.warning(f"Nina: Google Calendar init failed: {e}")
            self.calendar_service = None
    
    async def sync_to_calendar(self) -> Dict:
        """Sync current schedule to Google Calendar"""
        if not self.calendar_service:
            return {"success": False, "message": "📅 Calendar sync unavailable (no API connection)"}
        ...
        try:
            ...
            for i in range(7):
                ...
                if exception_data:
                    exception = json.loads(exception_data)
                    working = exception.get('working', False)
                    hours = exception.get('hours', {})
                else:
                    baseline = self.baseline_schedule.get(day_name, {})
                    working = baseline.get('working', False)
                    hours = {'start': baseline.get('start'), 'end': baseline.get('end')}
                
                if working and hours.get('start') and hours.get('end'):
                    # Create calendar event
                    event = {
                        'summary': '👩‍💼 Viola Working',
                        'description': 'Au pair on duty',
                        'start': {
                            'dateTime': f"{date_str}T{hours['start']}:00",
                            'timeZone': 'America/New_York',
                        },
                        'end': {
                            'dateTime': f"{date_str}T{hours['end']}:00",
                            'timeZone': 'America/New_York',
                        },
                        'colorId': '2',  # Green
                    }
                    ...
                    if not self._event_exists(date_str, 'Viola Working'):
            ...
            return {
                "success": True,
                "message": f"📅 Synced {synced} events to Google Calendar"
            }
            
        except Exception as e:
            logger.error(f"Calendar sync error: {e}")
            return {"success": False, "message": f"❌ Calendar sync failed: {str(e)[:50]}"}
    
    def _event_exists(self, date_str: str, summary: str) -> bool:
    ...
    async def check_conflicts(self, date_str: str = None) -> Dict:
        """Check for scheduling conflicts with family calendar"""
        if not self.calendar_service:
            return {"success": False, "conflicts": [], "message": "Calendar integration not available"}
        ...
        try:
            ...
            conflicts = []
            childcare_keywords = ['date', 'dinner', 'meeting', 'appointment', 'out']
            
            for event in events.get('items', []):
                summary = event.get('summary', '').lower()
                if any(kw in summary for kw in childcare_keywords) and 'viola' not in summary:
                    # Check if Viola is working
                    day_name = datetime.fromisoformat(date_str).strftime('%A').lower()
                    viola_working = await self._is_working_on_date(date_str, day_name)
            ...
            if conflicts:
                return {
                    "success": True,
                    "conflicts": conflicts,
                    "message": f"⚠️ Found {len(conflicts)} events needing coverage"
                }
            else:
                return {
                    "success": True,
            ...
            return {"success": False, "conflicts": [], "message": "Error checking conflicts"}
    
    async def _is_working_on_date(self, date_str: str, day_name: str) -> bool:
        """Check if Viola is working on a specific date"""
        # Check exceptions first
        if self.redis_client:
            exception_data = await self.redis_client.get(f"schedule:exceptions:{date_str}")
            if exception_data:
                exception = json.loads(exception_data)
                return exception.get('working', False)
        
        # Fall back to baseline
        return self.baseline_schedule.get(day_name, {}).get('working', False)
...
# Update the process_schedule_command function to handle new calendar commands
async def process_schedule_command(command: str, user_name: str = "User") -> Dict:
    """Process a schedule-related command (updated for Phase 2)"""
    if not nina_agent.redis_client:
        await nina_agent.initialize()
    ...
    if 'sync' in command_lower and 'calendar' in command_lower:
        return await nina_agent.sync_to_calendar()
    elif 'conflict' in command_lower or 'check calendar' in command_lower:
        # Extract date if provided
        date = nina_agent.parse_date_from_text(command)
        date_str = date.strftime('%Y-%m-%d') if date else None
        return await nina_agent.check_conflicts(date_str)
    # Original commands
    elif any(word in command_lower for word in ['what', 'show', 'check', 'when', 'schedule']):
        return await nina_agent.query_schedule(command)
    else:
        return await nina_agent.update_schedule(command, user_name)
