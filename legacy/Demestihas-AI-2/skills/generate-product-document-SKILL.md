...
## Core Purpose
When you request a document, this skill ensures you receive a complete, well-structured document directly in an artifact—not a conversational summary with "Here's what I created..." wrapper text. Documents are ALWAYS created as artifacts for easy viewing, editing, and downloading.
...
1. **ALWAYS Create Artifacts**: Every document MUST be created as an artifact, not inline text
2. **No Conversational Wrappers**: Start directly with the document content
3. **Complete by Default**: Include all standard sections even if not explicitly requested
4. **Professional Format**: Use proper Markdown with headers, tables, code blocks
5. **Context-Aware**: Use all prior conversation and existing artifact content
6. **Template Selection**: Choose the most appropriate template based on request
7. **Numbered Phases**: Use 1.1, 1.2, 2.1, 2.2 format, never time-based labels (weeks/days/hours)
